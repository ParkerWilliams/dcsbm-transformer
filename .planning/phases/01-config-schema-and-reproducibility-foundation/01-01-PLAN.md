...
must_haves:
  truths:
    ...
    - "run_experiment.py --config <path> loads a JSON config, prints summary, and exits (Phase 1 stub)"
  artifacts:
    ...
    - path: "run_experiment.py"
      provides: "CLI entry point with argparse --config flag"
      contains: "argparse"
  key_links:
...
- `make run`: invoke `python run_experiment.py --config config.json` (default config path)
...
**run_experiment.py** (project root — per CONTEXT.md locked decision: "Python script with argparse for custom invocations"):
- Use `argparse` with `--config <path>` argument (required, type=str, help="Path to experiment config JSON file")
- Add optional `--dry-run` flag that only loads and prints config without running
- In `main()`: parse args, read the JSON file at `--config` path, call `config_from_json()` to deserialize, print config summary (experiment_id format + config hash)
- This is a Phase 1 stub — the actual experiment execution logic will be added in later phases. Include a `# TODO: Phase 2+ — add graph generation, training, and evaluation pipeline` comment
- Must be executable: include `if __name__ == "__main__": main()` guard
- Import from `src.config`: `config_from_json`, `config_hash`, `full_config_hash`
- Import from `src.results`: `generate_experiment_id`
...
"</automated>
    <manual>Verify pyproject.toml has correct dependencies, Makefile has required targets, and run_experiment.py --help shows --config flag</manual>
  </verify>
  <done>
All 5 config dataclasses exist with frozen=True and slots=True. ANCHOR_CONFIG instantiates with locked defaults. Cross-parameter validation rejects invalid configs. Config hashing works with field exclusion. JSON round-trip preserves hash identity. run_experiment.py accepts --config flag and loads/prints config as a Phase 1 stub.
  </done>
</task>
