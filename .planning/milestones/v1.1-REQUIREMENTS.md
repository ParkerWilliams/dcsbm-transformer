# Requirements Archive: v1.1 Journal Feedback
...
- [x] **SFTX-01**: System includes a LaTeX derivation of the epsilon-bound from QK^T perturbation through softmax to AVWo spectral change, using the correct Lipschitz constant (1/2) and 1/sqrt(d_k) scaling — *docs/softmax_bound.tex*
- [x] **SFTX-02**: System empirically verifies the bound by injecting controlled perturbations into QK^T at specific steps and measuring actual AVWo spectral change vs theoretical bound — *src/analysis/perturbation_bound.py*
- [x] **SFTX-03**: System generates bound tightness visualization (theoretical envelope vs empirical measurements) and reports tightness ratio — *src/visualization/perturbation_bound.py*

### Multi-Head Ablation

- [x] **MHAD-01**: Transformer supports configurable n_heads (1, 2, 4) with per-head QK^T extraction; d_k is held constant (d_model scales as n_heads * d_k) — *src/model/attention.py*
- [x] **MHAD-02**: SVD metrics are computed per-head with NPZ keys in format `target.layer_N.head_H.metric_name`, with backward-compatible dual key emission — *fused_evaluate*
- [x] **MHAD-03**: System computes per-head AUROC and signal concentration analysis (entropy/Gini) — *src/analysis/signal_concentration.py*
- [x] **MHAD-04**: System runs ablation comparison on matched configs (1h/2h/4h, all d_k=128) — *ablation pipeline*
...
- [x] **PREG-01**: Pre-registration document committed with hypothesis, metric, alpha, correction, decision criterion — *docs/pre-registration.md*
- [x] **PREG-02**: System implements held-out evaluation split and tags results with split membership — *src/evaluation/split.py*
- [x] **PREG-03**: System maintains a deviation log tracking changes to analysis plan — *docs/deviation-log.md*

### Sharp Compliance Curve

- [x] **COMP-01**: System sweeps r/w ratio with fine granularity (8+ values, 3 seeds) — *src/analysis/compliance_curve.py*
- [x] **COMP-02**: System generates composite publication figure with dual y-axes — *src/visualization/compliance_curve.py*

### Full Spectrum Trajectory

- [x] **SPEC-01**: System stores top-k singular value vectors per step in NPZ (float16) — *fused_evaluate spectrum storage*
- [x] **SPEC-02**: System computes discrete Frenet-Serret curvature and torsion with Savitzky-Golay smoothing — *src/analysis/spectrum.py*
- [x] **SPEC-03**: Curvature and torsion fed into AUROC pipeline as exploratory metrics — *AUROC integration*
...
|-------------|-------|--------|
| PREG-01 | Phase 11 | Complete |
| PREG-02 | Phase 11 | Complete |
| PREG-03 | Phase 11 | Complete |
...
| SFTX-01 | Phase 14 | Complete |
| SFTX-02 | Phase 14 | Complete |
| SFTX-03 | Phase 14 | Complete |
...
| MHAD-01 | Phase 16 | Complete |
| MHAD-02 | Phase 16 | Complete |
| MHAD-03 | Phase 16 | Complete |
| MHAD-04 | Phase 16 | Complete |
